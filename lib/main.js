    ...
    return {
      ...
      lint (textEditor) {
        return new Promise((resolve, reject) => {
          ...
          const {stdout, stderr, error, status} = spawnSync(emacs, args, {encoding: 'utf8'})
          if (error) {
            warning('Emacs command failed', {error, args, stdout, stderr, status})
            resolve([])
            return
          }
          if (status !== 0) {
            warning('Emacs command didnt exit cleanly', {args, stdout, stderr, status})
            resolve([])
            return
          }
          try {
